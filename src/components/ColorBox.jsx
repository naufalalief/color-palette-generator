...
const ColorBox = ({ color, index, copiedIndex, onCopy }) => {
  return (
    <div
    ...
    >
      <p
        style={{ color: "#fff" }}
        className="text-center text-white text-lg font-bold bg-black/30 px-4 py-2 rounded-md"
      >
        {color}
      </p>
      <button
        onClick={() => onCopy(color, index)}
        className="absolute top-2 right-2 bg-white text-black font-bold py-1 px-2 rounded hover:bg-gray-200"
      >
        {copiedIndex === index ? "Copied" : copyIcon()}
      </button>
    </div>
  );
};
